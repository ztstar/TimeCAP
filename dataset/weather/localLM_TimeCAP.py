...
time_step = "hour"
domain = "weather"
target_event_description = "It will rain in the next 24 hours"
label_block = "rain or not rain"
...
def contextualize_whole(
    ...
    for i in indices[:10]: # TODO: remove :10
        output_path = f"{model}_summary/whole/{city}_{i}.txt"
        ...
        indicator_block = (
            ...
            f"- Humidity (%): {humidity}\n"
            f"- Air Pressure (hPa): {pressure}\n"
            f"- Wind Speed (m/s): {wind_speed}\n"
            f"- Wind Direction (degrees): {wind_direction}"
        )
...
def contextualize_channel(
    ...
    channels = [
        ("Humidity (%)", "humidity", 0),
        ("Air Pressure (hPa)", "airpressure", 1),
        ("Temperature (Kelvin)", "temperature", 2),
        ("Wind Speed (m/s)", "windspeed", 3),
        ("Wind Direction (degrees)", "winddirection", 4),
    ]
    ...
    for i in indices[:10]: # TODO: remove :10
        output_path = f"{model}_summary/channel/{city}_{i}.txt"
        if os.path.exists(output_path):
            print(f"Skipping {output_path} (already exists)")
            continue
...
def main():
    for city in ["ny"]: #  ,"hs", "sf"
        for model in ["qwen", "deepseek"]:
            contextualize_whole(city, model)
            contextualize_channel(city, model)
            # predict(city, model)
